...
def cocomo_II(dataset, a=2.94, b=0.91):
    ...
    for train, test in KFold_df(dataset, 3):
        input = test.iloc[:, :-2]
        kloc = test.iloc[:, -2:-1]
        effort_actual = test.iloc[:, -1]
        eaf = input.prod(axis=1).to_frame(name = 'eaf')
        temp = a * (kloc ** b)
        effort_predict = temp.iloc[:,0] * eaf.iloc[:,0]
        mre_list.append(mre_calc(effort_predict, effort_actual))   ######### for MRE
        sa_list.append(sa_calc(effort_predict, effort_actual))   ######### for SA
...
if __name__ == '__main__':
    print(cocomo_II(data_cocomo81())[0])